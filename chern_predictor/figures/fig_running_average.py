...
def gen_running_average_plot_data(
    ...
    with open(ensemble_path + "network_list.dat", "r") as file:
        model_name_list = json.load(file)

    weights_test = []
    for model_name in model_name_list:
        with open(os.path.join(model_path, model_name + "-evaluated.dat"), "r") as file:
    ...
    for i, n_avg in enumerate([2, 4, 8]):
        accs = []
        for epoch in range(n_epochs + 1 - n_avg):
            accs.append([])
            for n in range(n_nets):
                acc, _, _, _, _, _, _, _ = evaluate_ensemble(
    ...
    with open(os.path.join(figure_path, "fig_running_average_data.dat"), "w") as file:
        json.dump(figure_data, file)


def make_running_average_fig(figure_path: str, verbose: bool = False):

    with open(os.path.join(figure_path, "fig_running_average_data.dat"), "r") as file:
        figure_data = json.load(file)

    def plot_entry(n_epochs, accs, ax1, ax2, color, name, ymin, ymax, n_bins=45):
        ...
        accs = np.array(accs)[evaluation_params["epochs_until_trained"] + 1 :].flatten()
    ...
    for ax in axes[[1]]:
        ax.set_xlim(0, 1.0 * overall_max_count)
        ax.set_ylim(ymin, ymax)
    for ax in axes[[0, 1]]:
    ...
    for ax in axes[[0]]:
        ...
        for ensemble_size in evaluation_params["ensemble_sizes"]:
            ax.vlines(
                [evaluation_params["epochs_until_trained"] + ensemble_size - 1 + 0.5],
                0,
                1,
                linestyle="dashed",
                color="gray",
