...
def gen_performance_plot_data(
    ...
    for prepare in [True, False]:
        for n, ensemble_size in enumerate(evaluation_params["ensemble_sizes"]):
            ...
            if prepare:
                ...
                if max_count < np.max(counts):
                    max_count = np.max(counts)
            else:
                data["training"][f"ensemble_size_{ensemble_size}"] = {}
                data["histogram"][f"ensemble_size_{ensemble_size}"] = {}
    ...
    with open(os.path.join(figure_path, "fig_performance_data.dat"), "w") as file:
        json.dump(data, file)
...
def make_performance_plots(
    ...
    with open(os.path.join(figure_path, "fig_performance_data.dat"), "r") as file:
    ...
    for n, ensemble_size in enumerate(evaluation_params["ensemble_sizes"]):
        ...
        max_count = figure_data["histogram"][f"ensemble_size_{ensemble_size}"][
            "max_count"
        ]
        ...
        scaling_factor = 140.0 / max_count
        ax_histogram.hist(
            accs,
            bins=bins,
            color=colors[n],
            weights=scaling_factor / (len(accs) * step_size) * np.ones(len(accs)),
            alpha=0.4,
            orientation="horizontal",
        )
        xs = np.linspace(ymin, ymax, 201)
        ys = scaling_factor * norm.pdf(xs, loc=np.mean(accs), scale=np.std(accs))
