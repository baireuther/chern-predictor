...
if __name__ == "__main__":
    ...
    num_training_points = 12500
    num_validation_points = 1250
    num_test_points = 1250
    ...
    if generate_data:
        ...
        params["dataset"]["num_hams_per_chern_number"] = num_training_points
        params["dataset"]["smallest_ham_seed"] = 0 * 10**8
        training_dataset = gen_dataset(
            params=params, dataset_path=os.path.join(dataset_directory, "training.dat")
        )
        ...
        params["dataset"]["num_hams_per_chern_number"] = num_validation_points
        params["dataset"]["smallest_ham_seed"] = 1 * 10**8
        validation_dataset = gen_dataset(
            params=params,
            dataset_path=os.path.join(dataset_directory, "validation.dat"),
        )
        print(
        ...
        params["dataset"]["num_hams_per_chern_number"] = num_test_points
        params["dataset"]["smallest_ham_seed"] = 2 * 10**8
        test_dataset = gen_dataset(
            params=params, dataset_path=os.path.join(dataset_directory, "test.dat")
        )
    ...
    for ensemble_name in ["cnn32", "cnn64", "cnn64dl"]:
        ...
        if analyze_data:
            ...
            meta_data_path = "data_analysis/resources/"
            ...
            (
                ...
                title="Localization length estimates",
                filepath=os.path.join(
                    figure_path, "localization_length_fits_training_data"
                ),
                plotting=True,
                verbose=True,
            )
            print(
                f"Training dataset: The localization length estimated by a fit starting {offset} sites "
                f"from the boundary to the center is {loc_length_center}."
            )
            ...
            (
                ...
                title="Localization length estimates",
                filepath=os.path.join(
                    figure_path, "localization_length_fits_test_data"
                ),
                plotting=True,
                verbose=True,
            )
            print(
                f"Test dataset: The localization length estimated by a fit starting {offset} sites "
                f"from the boundary to the center is {loc_length_center}."
            )
            print(
